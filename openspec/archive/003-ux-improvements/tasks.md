...
- [x] 0.1 创建 feature-003-ux-improvements 分支
  - `git checkout -b feature-003-ux-improvements`

- [x] 0.2 创建 tests/detail.test.ts 测试文件
...
- [x] 1.1 编写测试: 验证 setupCopyButton 函数存在
  - 预期失败: "setupCopyButton is not defined"

- [x] 1.2 运行测试确认失败
  - 看到预期失败信息

- [x] 1.3 实现 setupCopyButton 函数（空函数）
  - 测试通过（函数存在）

- [x] 1.4 编写测试: 验证点击按钮调用 clipboard.writeText
  - 预期失败: clipboard 未被调用
...
- [x] 1.7 编写测试: 验证成功后调用 showNotification
  - 预期失败: 通知未显示
...
- [x] 1.10 在 init() 中调用 setupCopyButton
  - 测试通过

- [x] 1.11 编写测试: 验证剪贴板错误时显示错误通知
  - 预期失败: 错误处理未实现
...
- [x] 2.1 编写测试: 验证截图成功后创建通知
  - 预期失败: notifications.create 未被调用

- [x] 2.2 运行测试确认失败
  - 看到预期失败

- [x] 2.3 在 captureVisibleTab 成功后添加通知调用
  - 测试通过

- [x] 2.4 创建 showSuccessNotification 辅助函数
  - 代码复用，测试通过

- [x] 2.5 编写测试: 验证通知包含正确的消息
  - 预期失败: 消息内容不正确
...
- [x] 3.1 编写测试: 验证 OCR 完成后创建通知
  - 预期失败

- [x] 3.2 运行测试确认失败
  - 看到预期失败

- [x] 3.3 在 writeToClipboardViaOffscreen 成功后添加通知
  - 测试通过

- [x] 3.4 编写测试: 验证通知消息包含 "clipboard"
  - 预期失败: 消息不正确
...
- [x] 4.1 修改 HTML: 添加 data-history-nav 容器
  - DOM 结构更新

- [x] 4.2 修改 HTML: 将 left-section 改为 right-section
  - 截图容器移到右侧

- [x] 4.3 修改 HTML: 创建 data-middle-section 包含文本和按钮
  - 文本和按钮居中

- [x] 4.4 更新 CSS: 三栏 flex 布局
  - 布局正确显示

- [x] 4.5 更新 CSS: 历史导航 180px 宽度
  - 导航栏宽度正确

- [x] 4.6 更新 CSS: 历史项样式（悬停、激活）
  - 样式正确

- [x] 4.7 更新 CSS: 中间栏和右栏 flex: 1
  - 两栏平均分配剩余空间
...
- [x] 5.3 实现 formatTimestamp 函数
  - 时间格式化正确（"Just now", "5m ago", "2h ago"）

- [x] 5.4 编写测试: 验证 formatTimestamp 边界情况
  - 预期失败: 边界情况未处理
...
- [x] 5.7 实现 renderHistoryNavigation 函数
  - 测试通过，列表渲染
...
- [x] 5.10 实现当前项高亮逻辑（基于 URL 参数）
  - 功能已在 renderHistoryNavigation 中实现
  - .active 类正确应用到当前项

- [x] 5.11 在 init() 中调用 renderHistoryNavigation
  - 页面加载时渲染
...
- [x] 6.1 编写测试: 验证点击历史项更新内容
  - 预期失败: 内容未更新

- [x] 6.2 运行测试确认失败
  - 内容未更新

- [x] 6.3 实现 loadHistoryItemDynamic 函数
  - 测试通过

- [x] 6.4 更新 URL (pushState) 无刷新
  - URL 更新
...
- [x] 6.7 编写测试: 验证浏览器历史记录
  - 预期失败: pushState 未被调用

- [x] 6.8 运行测试确认失败
  - pushState 未被调用

- [x] 6.9 验证 pushState 被正确调用
  - 测试通过
...
- [x] 7.1.3 实现 escapeHtml 函数
  - 转义 &, <, >, ", '
  - 函数已实现在 src/detail/main.ts:516-525

- [x] 7.1.4 修改 simpleMarkdownParse 先转义 HTML
  - 所有 markdown 处理前先转义
  - simpleMarkdownParse 在第 537 行调用 escapeHtml

- [x] 7.1.5 编写测试: img 标签 onerror 事件被转义
...
- [x] 7.1.6 运行测试确认通过
  - XSS 被阻止
  - 10 out of 11 XSS 测试通过（除了 javascript: 链接过滤）

- [x] 7.1.7 编写测试: javascript: 链接被过滤
...
- [x] 7.1.11 编写测试: 链接包含 rel="noopener noreferrer"
...
- [x] 7.1.13 实现链接安全属性
  - 添加 rel 和 target
  - 功能已在 src/detail/main.ts:566 实现 ✓
...
- [x] 7.2.7 扩展 simpleMarkdownParse 函数
  - 保持 XSS 安全，添加新语法支持
  - 保护代码块不被处理
  - 实现列表、链接、引用、分隔线
...
- [x] 7.2.9 导出 simpleMarkdownParse 用于测试
  - 测试可访问
...
- [x] 8.1 更新 package.json 版本到 0.6.0
  - `"version": "0.6.0"`

- [x] 8.2 更新 public/manifest.json 版本到 0.6.0
  - `"version": "0.6.0"`

- [x] 8.3 运行 npm run build
  - 构建成功，无错误

- [x] 8.4 验证 dist/manifest.json 版本
  - 版本为 0.6.0
